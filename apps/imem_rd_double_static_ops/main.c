 ...
 * */
 ...
 int main(void)
 {
    ...
    //  if (__ctx() == 0) {
         ...
         __declspec(ctm shared) __mem40 uint32_t *data_2;

         // Sequential access variables - replaces hash-based access
         __gpr uint32_t access_counter = 0;  // Thread-local counter, increments each packet
         __gpr uint32_t access_counter_2 = 20000;  // Thread-local counter, increments each packet
         __gpr uint32_t bucket_idx;
         __gpr uint32_t entry_idx;
         __gpr uint32_t accessed_value;
         __gpr uint32_t accessed_value_2;
         ...
         for (;;) {
             ...
             data_2 = (__mem40 uint32_t *)(pbuf + pkt_off
                                              + sizeof(struct eth_hdr)
                                              + sizeof(struct ip4_hdr)
                                              + sizeof(struct udp_hdr)
                                              + sizeof(uint32_t));
             ...
             // Sequential access pattern - no hash-based lookup
             // Calculate bucket and entry from access counter (inline)
             bucket_idx = (access_counter / CONN_TABLE_MAX_KEYS_PER_BUCKET) % CONN_TABLE_NUM_BUCKETS;
             entry_idx = access_counter % CONN_TABLE_MAX_KEYS_PER_BUCKET;

             // Directly read the calculated bucket and entry
             accessed_value = conn_table[bucket_idx].four_tuple_hash_entry[entry_idx];

             bucket_idx = (access_counter_2 / CONN_TABLE_MAX_KEYS_PER_BUCKET) % CONN_TABLE_NUM_BUCKETS;
             entry_idx = access_counter_2 % CONN_TABLE_MAX_KEYS_PER_BUCKET;
             accessed_value_2 = conn_table[bucket_idx].four_tuple_hash_entry[entry_idx];

             // Write to packet data
             *data = accessed_value;
             *data_2 = accessed_value_2;

             // Increment access counter for next packet
            //  access_counter++;
