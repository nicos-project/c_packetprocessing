 ...
 * */
 ...
 #define CONN_TABLE_NUM_BUCKETS 16384*512
 ...
 __declspec(emem0 export scope(global)) struct conn_table_bucket conn_table[CONN_TABLE_NUM_BUCKETS];
...
//  __declspec(imem export scope(global)) uint8_t ct_bucket_count[CONN_TABLE_NUM_BUCKETS];
 ...
 int main(void)
 {
    ...
    //  if (__ctx() == 0) {
         ...
         // xorshift128+ RNG state (per-context) - better quality than xorshift32
         __gpr uint32_t rng_s0, rng_s1, rng_s2, rng_s3;
         __gpr uint32_t seed_base;
         __gpr uint32_t t, s;
         __gpr uint64_t m;

         island = __ISLAND;

         // Initialize RNG once per context with different seeds
         seed_base = local_csr_read(local_csr_timestamp_low);
         rng_s0 = seed_base ^ ((__ctx() << 8) + 0x9E3779B9);
         rng_s1 = seed_base ^ ((__ME() << 16) + 0x85EBCA6B);
         rng_s2 = seed_base ^ ((island << 24) + 0xC2B2AE35);
         rng_s3 = seed_base ^ 0x27D4EB2F;

         // Prevent all-zero state (xorshift128+ degenerates if all state is zero)
         if ((rng_s0 | rng_s1 | rng_s2 | rng_s3) == 0) {
             rng_s0 = 0x12345678;
             rng_s1 = 0x9ABCDEF0;
             rng_s2 = 0x13579BDF;
             rng_s3 = 0x2468ACE0;
         ...
         for (i = 0; i < CONN_TABLE_NUM_BUCKETS; i++) {
             for (j = 0; j < CONN_TABLE_MAX_KEYS_PER_BUCKET; j++) {
                 conn_table[i].four_tuple_hash_entry[j] = i * CONN_TABLE_NUM_BUCKETS + j;
             }
         }
         ...
         for (;;) {
             ...
             // xorshift128+ algorithm - high quality RNG
             t = rng_s1;
             s = rng_s0;
             rng_s0 = rng_s1;
             rng_s1 = rng_s2;
             ...
             t ^= t >> 8;
             rng_s3 = t ^ s ^ (s >> 19);
             random_value = rng_s0 + rng_s3;  // xorshift128+ output

             // Safety check: prevent all-zero state (should never happen mathematically, but be safe)
             if ((rng_s0 | rng_s1 | rng_s2 | rng_s3) == 0) {
                 rng_s0 = 0x12345678;
                 rng_s1 = 0x9ABCDEF0;
                 rng_s2 = 0x13579BDF;
             ...
             // Lemire's fast unbiased range mapping (avoids modulo bias)
             // Instead of: bucket_idx = random_value % CONN_TABLE_NUM_BUCKETS
             m = (uint64_t)random_value * (uint64_t)CONN_TABLE_NUM_BUCKETS;
             bucket_idx = m >> 32;  // Upper 32 bits give unbiased result

             // Extract entry index from different random bits
             entry_idx = (rng_s1 >> 2) & 0x3;  // Use fresh random bits for entry (0-3)
